---
name: stack-nextjs
description: Use ONLY when o projeto usa Next.js (App Router). Cobre stack canônica (Next 15+, React 19+, TypeScript, Tailwind, Zod), comandos, estrutura de pastas, Server Components, Server Actions, validação com Zod, performance, acessibilidade e testes com Vitest + Testing Library.
---

# Stack — Next.js (App Router)

> Adendo ao workflow principal. Use **apenas** se o projeto for Next.js com App Router.
...
**Regra**: código específico de feature mora em `src/features/<feature>/`. Extraia para `lib/` ou `components/ui/` apenas quando reutilizado por **2+ features**.
...
- Use `revalidateTag` em mutações que afetam listas cacheadas.
...
## 10. Bootstrap (uma vez por projeto)
...
- ❌ `useEffect` para data fetching quando pode usar Server Component.
- ❌ Server Action mutando sem `revalidateTag`/`revalidatePath`.
- ❌ Importar código `'use server'` em Client Component (exceto chamar a action).
...
- ❌ `'use client'` no topo de layouts/páginas sem necessidade real.
