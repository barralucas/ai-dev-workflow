---
name: stack-node
description: Use ONLY when o projeto é um backend Node.js (Fastify, Express ou NestJS) com TypeScript. Cobre stack canônica, comandos, estrutura de pastas por domínio, validação com Zod, padrão de erro, logs com pino, auth JWT, background jobs e testes com Vitest + supertest.
---

# Stack — Node.js Backend

> Adendo ao workflow principal. Use para serviços HTTP/jobs em Node.js com TypeScript.
...
- Hierarquia: `DomainError` → `NotFoundError`, `ConflictError`, `UnauthorizedError`.
...
- **Integração**: routes + DB de teste (testcontainers ou SQLite em memória).
...
- ❌ `try { ... } catch (e) { console.log(e) }` — use logger estruturado e re-lance.
...
- ❌ `setInterval` para jobs — use BullMQ/pg-boss.
