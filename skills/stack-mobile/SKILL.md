---
name: stack-mobile
description: Use ONLY when o projeto é um app mobile com React Native / Expo. Cobre stack canônica, comandos, estrutura de pastas, expo-router, TanStack Query, Zod, acessibilidade mobile, performance com FlatList, EAS Build, testes com Jest + RNTL e segurança específica de mobile.
---

# Stack — Mobile (React Native / Expo)

> Adendo ao workflow principal. Use para apps mobile cross-platform.
...
- Toda resposta de API valida com **Zod** antes de usar (não confie nos tipos do backend).
...
## 10. Segurança Específica de Mobile

- **Nunca** embutir chaves de API no bundle (apps são engenharia-reversáveis).
- Tokens em **Secure Store** (Keychain/Keystore), nunca em `AsyncStorage`.
- Certificate pinning para endpoints sensíveis.
